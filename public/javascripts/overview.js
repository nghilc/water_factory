...
//     $('#filter_danhsachtram').selectpicker('val', localStorage.getItem('org_id')).trigger('change');
...
$(document).ready(function () {
  $("#filter_danhsachtram").change(function () {
    ...
    if (type == "nha_may") {
      $("#chi_so_moi_truong_card").css("display", "block")
      $("#danh_sach_nha_may_card").css("display", "none")
      get_data_nha_may(val);
    } else {
      $.ajax({
        url: "/overview/get/danhsachnhamay",
        type: "GET",
        data: { org_id: val, type },
        beforeSend: function () {
          showLoading();  // Hiện overlay khi bắt đầu tải
        ...
        success: function (res) {
          $("#chi_so_moi_truong_card").css("display", "none")
          $("#danh_sach_nha_may_card").css("display", "block")
          $("#danh_sach_nha_may_contaier").empty();
          let danh_sach_nha_may = res.danh_sach_nha_may;
          let str = "";
          for (let i = 0; i < danh_sach_nha_may.length; i++) {
            ...
            <div class="panel-actions">
              <button class="btn-overview truy_cap_nha_may" org_id="${danh_sach_nha_may[i].id_nha_may}" style="display: block;">Vào giám sát</button>
            </div>
            </div>
            `
          }
          $("#danh_sach_nha_may_contaier").append(str);
...
function get_data_nha_may(val) {
  $.ajax({
    url: "/overview/get/dulieunhamay",
    type: "GET",
    data: { org_id: val },
    beforeSend: function () {
      showLoading();  // Hiện overlay khi bắt đầu tải
...
function render_table_meter_list(meter_list) {
  ...
  for (let i = 0; i < meter_list.length; i++) {
    x += '<tr MeterCode="' + meter_list[i].MeterCode + '" NodeCode="' + meter_list[i].NodeCode + '" meter_name="' + meter_list[i].meter_name + '" meter_status="' + meter_list[i].status_meter + '" location_lat="' + meter_list[i].lat + '" location_long="' + meter_list[i].lng + '" type="' + meter_list[i].meter_type + '" data_type="' + meter_list[i].data_type + '"><td class="text-left">' + meter_list[i].meter_name + '</td>';
...
$(document).ready(function () {
  ...
  $("#searchInput").on("keyup", function () {
    ...
    $("#danhsachtram tbody tr").filter(function () {
      $(this).toggle($(this).attr("MeterCode").toLowerCase().indexOf(value) > -1 || $(this).text().toLowerCase().indexOf(value) > -1);
      // $(this).toggle();
    });
  });
})
