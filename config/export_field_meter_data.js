...
function return_meter_status(data) {
    ...
    if (data.data_field_config) {
        ...
        data_field_config = JSON.stringify({ "ValOfNum": 1, "terminal_index": 1, "flowRate": 1, "pressure": 1, "measure_sensor": 1, "measure_static": 1, "measure_dynamic": 1, "measure_delta": 1, "ValueReverse": 1, "Voltage": 1, "voltage_ac_quy": 1, "PressureVol": 1 })
    }
...
async function export_data(req, res) {
    try {
        ...
        let name_file = "BAO_CAO_DU_LIEU_" + toConstFormat(shortToFullName(key))+"_" + start_date + "_" + end_date
        let data = [];
        data = await query(`SELECT * FROM "Operation" WHERE "NodeCode" = $1 AND "MeterCode" = $2 AND "MeterTime" >= $3 AND "MeterTime" <= $4 ORDER BY "MeterTime" DESC;`, [NodeCode, MeterCode, start_date, end_date]);
            let data_field_config = JSON.stringify({ "ValOfNum": 1, "terminal_index": 1, "flowRate": 1, "pressure": 1, "measure_sensor": 1, "measure_static": 1, "measure_dynamic": 1, "measure_delta": 1, "ValueReverse": 1, "Voltage": 1, "voltage_ac_quy": 1, "PressureVol": 1 })
            ...
            worksheet.getCell('A2').value = "BÁO CÁO DỮ LIỆU " + (shortToFullName(key)).toUpperCase();
            ...
            worksheet.getCell('C9').value = shortToFullName(key);
            ...
            for (let i = 0; i < result.length; i++) {
                ...
                worksheet.getCell("C" + (10 + i)).value = show_if_null_text((typeof result[i][key] !== 'undefined') ? (result[i][key]) : "-");
                if (typeof result[i][key] !== 'undefined') {
                    worksheet.getCell('C' + (10 + i)).numFmt = '#,##0.00'; // định dạng số
                }
            }
