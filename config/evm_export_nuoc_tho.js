...
async function export_data(req, res) {
    try {
        ...
        let data = await access_db("SELECT MeterTime as time, DoDuc as do_duc, Temp as nhiet_do,PH as ph, NULL as do_cung FROM web_environment.Operation WHERE MeterCode = ?	AND MeterTime >= ? AND MeterTime <= ? ORDER BY MeterTime", [MeterCode, start_date, end_date]);
        ...
        worksheet.getCell('A2').value = "BÁO CÁO CHỈ SỐ TỪNG THỜI ĐIỂM NƯỚC THÔ";
