...
function exportEQ(req,res){
  ...
  if(req.user.role == "admin"){
    sql = "SELECT *, totaleq.name AS meter_name FROM totaleq INNER JOIN tram_diemdo_relation ON totaleq.NodeCode = tram_diemdo_relation.NodeCode AND totaleq.MeterCode = tram_diemdo_relation.MeterCode WHERE tram_diemdo_relation.parent_id = ?;"
    sql_val = [node_id];
  }else{
    sql = "SELECT *, totaleq.name AS meter_name FROM totaleq INNER JOIN tram_diemdo_relation ON totaleq.NodeCode = tram_diemdo_relation.NodeCode AND totaleq.MeterCode = tram_diemdo_relation.MeterCode INNER JOIN group_user ON totaleq.MeterCode = group_user.ideq WHERE group_user.user_id = ? AND tram_diemdo_relation.parent_id = ?;";
    sql_val = [user_id, node_id];
  }
  accessDB(sql,sql_val, async function(result){
    ...
    worksheet.getCell('G10').value = "TSS";
    ...
    worksheet.getCell('H10').value = "Clo (mg/l)";
    ...
    worksheet.getCell('I10').value = "Độ đục (NTU)";
    ...
    worksheet.getCell('J10').value = "Vận hành";
    ...
    worksheet.getCell('K10').value = "Trạng thái";
    worksheet.getCell('K10').font ={name: 'Arial', size: 10, bold: true };
    worksheet.getCell('K10').alignment = { vertical: 'middle', horizontal: 'center', wrapText : true };

    for(let i=0; i<result.length; i++){
      worksheet.getCell('A'+(11+i)).value = return_if_null(result[i].MeterCode);
      worksheet.getCell('B'+(11+i)).value = return_if_null(result[i].meter_name);
      ...
      worksheet.getCell('G'+(11+i)).value = 12;
      worksheet.getCell('H'+(11+i)).value = 0.3;
      worksheet.getCell('I'+(11+i)).value = 2.5;
      worksheet.getCell('J'+(11+i)).value = return_operte(result[i].last_Operate);
      worksheet.getCell('K'+(11+i)).value = return_status(result[i].last_status);
