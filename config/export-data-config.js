...
function exportData(req,res,info,status){
    // let MeterCode = req.session.ideq;
    let from = req.query.fr;
    let to = req.query.to;
    // let node_code = req.session.node_code
    let node_id = req.query.node_id;
    let name_file = "CHI_SO_TUNG_THOI_DIEM"+return_date(from)+"_"+return_date(to);
    accessDB("SELECT * FROM Operation INNER JOIN tram_diemdo_relation ON Operation.MeterCode = tram_diemdo_relation.MeterCode AND Operation.NodeCode = tram_diemdo_relation.NodeCode WHERE tram_diemdo_relation.id = ? AND Operation.MeterTime > ? AND Operation.MeterTime < ? ORDER BY MeterTime DESC",[node_id,returnSQLDateFormat(from),returnSQLDateFormat(to)],async function(data){
            var result = JSON.parse(JSON.stringify(data));
            let workbook = new excel.Workbook(); //creating workbook
            let worksheet = workbook.addWorksheet(info.MeterCode); //creating worksheet
            ...
            worksheet.getCell('A4').value = "CHỈ SỐ TỪNG THỜI ĐIỂM";
            ...
            worksheet.getCell('A9').value = "Có "+ result.length +" bản ghi từ "+ return_local_date_string(new Date(Number(from))) +" đến "+ return_local_date_string(new Date(Number(to)));
            ...
            worksheet.getCell('C10').value = "Nhiệt độ (°C)";
            ...
            worksheet.getCell('D10').value = "Độ PH";
            ...
            worksheet.getCell('E10').value = "Oxy (mg/l)";
            ...
            worksheet.getCell('F10').value = "Độ mặn (ppt)";
            ...
            worksheet.getCell('G10').value = "Oxy (%)";
            ...
            for(let i=0; i<result.length; i++){
              ...
              worksheet.getCell('B'+(11+i)).value =  return_local_date_string(new Date(result[i].MeterTime));
              worksheet.getCell('B'+(11+i)).alignment = { vertical: 'middle', horizontal: 'center', wrapText : true };

              worksheet.getCell('C'+(11+i)).value =  fixed_data(result[i].Temp);
              worksheet.getCell('D'+(11+i)).value =  fixed_data(result[i].PH);
              worksheet.getCell('E'+(11+i)).value =  fixed_data(result[i].DO);
              worksheet.getCell('F'+(11+i)).value =  fixed_data(result[i].Salinity);
              worksheet.getCell('G'+(11+i)).value =  fixed_data(result[i].ORP);
            }
