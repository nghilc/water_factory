...
async function export_data(req, res) {
    try {
        ...
        worksheet.getCell('A2').value = "BÁO CÁO CHỈ SỐ TỪNG THỜI ĐIỂM ĐIỆN ÁP LƯỚI";
        ...
        worksheet.getCell('E8').value = "Tần số";
        ...
        worksheet.getCell('F8').value = "Dây";
        ...
        worksheet.getCell('G8').value = "Điện áp";
        ...
        for (let i = 0; i < result.length; i++) {
            ...
            worksheet.getCell('C' + (9 + i*3)).value = "Pha A"
            worksheet.getCell('C' + (9 + i*3)).alignment = { vertical: 'middle', horizontal: 'left', wrapText: true };

            worksheet.getCell('C' + (9 + i * 3 +1)).value = "Pha B"
            worksheet.getCell('C' + (9 + i * 3 + 1)).alignment = { vertical: 'middle', horizontal: 'left', wrapText: true };

            worksheet.getCell('C' + (9 + i * 3+2)).value = "Pha C"
            worksheet.getCell('C' + (9 + i * 3 + 2)).alignment = { vertical: 'middle', horizontal: 'left', wrapText: true };

            worksheet.getCell('D' + (9 + i * 3)).value = show_if_null(result[i].V_A2_v)
            worksheet.getCell('D' + (9 + i * 3)).alignment = { vertical: 'middle', horizontal: 'right', wrapText: true };

            worksheet.getCell('D' + (9 + i * 3 +1)).value = show_if_null(result[i].V_B2_v)
            worksheet.getCell('D' + (9 + i * 3+1)).alignment = { vertical: 'middle', horizontal: 'right', wrapText: true };

            worksheet.getCell('D' + (9 + i * 3+2)).value = show_if_null(result[i].V_C2_v)
            worksheet.getCell('D' + (9 + i * 3+2)).alignment = { vertical: 'middle', horizontal: 'right', wrapText: true };

            worksheet.mergeCells('E' + (9 + i * 3) + ':E' + (9 + i * 3 + 2));
            worksheet.getCell('E' + (9 + i * 3)).value = show_if_null(result[i].Freq2_v)
            worksheet.getCell('E' + (9 + i * 3)).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };

            worksheet.getCell('F' + (9 + i * 3)).value = "AB"
            worksheet.getCell('F' + (9 + i * 3)).alignment = { vertical: 'middle', horizontal: 'left', wrapText: true };

            worksheet.getCell('F' + (9 + i * 3 + 1)).value = "BC"
            worksheet.getCell('F' + (9 + i * 3 + 1)).alignment = { vertical: 'middle', horizontal: 'left', wrapText: true };

            worksheet.getCell('F' + (9 + i * 3 + 2)).value = "CA"
            worksheet.getCell('F' + (9 + i * 3 + 2)).alignment = { vertical: 'middle', horizontal: 'left', wrapText: true };

            worksheet.getCell('G' + (9 + i * 3)).value = show_if_null(result[i].V_AB2_v)
            worksheet.getCell('G' + (9 + i * 3)).alignment = { vertical: 'middle', horizontal: 'right', wrapText: true };

            worksheet.getCell('G' + (9 + i * 3 + 1)).value = show_if_null(result[i].V_BC2_v)
            worksheet.getCell('G' + (9 + i * 3 + 1)).alignment = { vertical: 'middle', horizontal: 'right', wrapText: true };

            worksheet.getCell('G' + (9 + i * 3 + 2)).value = show_if_null(result[i].V_CA2_v)
            worksheet.getCell('G' + (9 + i * 3 + 2)).alignment = { vertical: 'middle', horizontal: 'right', wrapText: true };
