...
async function export_data(req,res){
    try {
        ...
        let data = await query(`SELECT "MeterTime" as "time", "measure_sensor", "measure_dynamic" FROM "Operation" WHERE "MeterCode" = $1 AND "MeterTime" >= $2 AND "MeterTime" <= $3 ORDER BY "MeterTime"`, [MeterCode, start_date, end_date]);
        ...
        worksheet.getCell('C8').value = "Mực nước hiện tại";
        ...
        worksheet.getCell('D8').value = "Mực nước động";
        ...
        worksheet.getCell('D8').alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          ...
          for(let i=0; i<result.length; i++){
            ...
            worksheet.getCell('C' + (9 + i)).value = show_if_null(result[i].measure_sensor)
              worksheet.getCell('C' + (9 + i)).alignment = { vertical: 'middle', horizontal: 'right', wrapText: true };

            worksheet.getCell('D' + (9 + i)).value = show_if_null(result[i].measure_dynamic)
